...
const defaults = {
  site: { schoolName:'Sekolah Kebangsaan Obah', shortName:'SK Obah', tagline:'Beluran, Sabah', motto:'Berilmu, Berbakti, Berjaya', facebookUrl:'', phone:'', email:'', address:'Beluran, Sabah', totalStudents:0, totalTeachers:0, totalStaff:0, totalPreschool:0, schoolAchievement:'Maklumat pencapaian sekolah akan dikemas kini.' },
...
function bindChrome(){
  ...
  const dashboard=document.querySelector('#dashboard'),statistics=document.querySelector('.school-statistics'),leadership=document.querySelector('.leadership-section');if(dashboard&&statistics&&leadership)leadership.before(dashboard,statistics);
}
...
const statisticObserver='IntersectionObserver'in window?new IntersectionObserver(entries=>entries.forEach(entry=>{if(!entry.isIntersecting)return;entry.target.dataset.statVisible='true';statisticObserver.unobserve(entry.target);animateStatistic(entry.target,entry.target.dataset.statTarget)}),{threshold:.35}):null;
function animateStatistic(el,value){const target=Math.max(0,Math.round(Number(value)||0));el.dataset.statTarget=String(target);if(el.dataset.statVisible!=='true'&&statisticObserver){statisticObserver.observe(el);return}if(window.matchMedia('(prefers-reduced-motion: reduce)').matches){el.textContent=target.toLocaleString('ms-MY');return}const old=statisticAnimations.get(el);if(old)cancelAnimationFrame(old);const startValue=Number(el.textContent.replace(/[^\d]/g,''))||0,startTime=performance.now(),duration=900;const tick=now=>{const progress=Math.min(1,(now-startTime)/duration),eased=1-Math.pow(1-progress,3),current=Math.round(startValue+(target-startValue)*eased);el.textContent=current.toLocaleString('ms-MY');if(progress<1)statisticAnimations.set(el,requestAnimationFrame(tick));else statisticAnimations.delete(el)};statisticAnimations.set(el,requestAnimationFrame(tick))}
function updateStatistics(site){document.querySelectorAll('[data-stat-value]').forEach(el=>{const field=statisticFields[el.dataset.statValue];if(!field)return;const value=Math.max(0,Math.round(Number(site[field])||0));el.dataset.statTarget=String(value);el.textContent=value.toLocaleString('ms-MY');if(el.dataset.statVisible==='true')animateStatistic(el,value)})}
function applySite(x={}){const s={...defaults.site,...x},root=document.documentElement,validColor=v=>/^#[0-9a-f]{6}$/i.test(v||'');document.querySelectorAll('[data-school-name]').forEach(e=>e.textContent=s.schoolName);document.querySelectorAll('[data-tagline]').forEach(e=>e.textContent=s.tagline);document.querySelectorAll('[data-motto]').forEach(e=>e.textContent=s.motto);document.querySelectorAll('[data-address]').forEach(e=>e.textContent=s.address);document.querySelectorAll('[data-phone]').forEach(e=>e.textContent=s.phone||'Belum ditetapkan');document.querySelectorAll('[data-email]').forEach(e=>e.textContent=s.email||'Belum ditetapkan');document.querySelectorAll('[data-school-achievement]').forEach(e=>e.textContent=s.schoolAchievement||defaults.site.schoolAchievement);const vision=document.querySelector('#publicVision'),mission=document.querySelector('#publicMission'),motto=document.querySelector('#publicMotto');if(vision&&s.vision)vision.textContent=s.vision;if(mission&&s.mission)mission.textContent=s.mission;if(motto&&s.motto)motto.textContent=s.motto;document.querySelectorAll('[data-facebook]').forEach(e=>{if(s.facebookUrl){e.href=safeUrl(s.facebookUrl);e.hidden=false}else e.hidden=true});if(validColor(s.primaryColor))root.style.setProperty('--blue',s.primaryColor);if(validColor(s.secondaryColor))root.style.setProperty('--green',s.secondaryColor);if(validColor(s.accentColor))root.style.setProperty('--yellow',s.accentColor);root.style.setProperty('--hero-overlay',String(Math.min(90,Math.max(20,Number(s.heroOverlay||65)))/100));if(s.bannerUrl)root.style.setProperty('--hero-image',`url("${safeUrl(s.bannerUrl)}")`);const section=document.body.dataset.section,sectionKey=section?`${section}BannerUrl`:'';if(sectionKey&&s[sectionKey])root.style.setProperty('--page-hero-image',`url("${safeUrl(s[sectionKey])}")`);if(s.logoUrl)document.querySelectorAll('.brand img,[data-site-logo]').forEach(e=>e.src=safeUrl(s.logoUrl));if(s.flagUrl)document.querySelectorAll('[data-site-flag]').forEach(e=>e.src=safeUrl(s.flagUrl));if(s.planUrl)document.querySelectorAll('[data-site-plan]').forEach(e=>e.src=safeUrl(s.planUrl));updateStatistics(s);}
...
function renderModules(section,items){const el=document.querySelector('[data-module-rail]');if(!el)return;const rows=items.map(x=>Array.isArray(x)?{title:x[0],description:x[1],icon:x[2]}:x).filter(x=>x.active!==false).sort((a,b)=>(a.order||0)-(b.order||0));el.innerHTML=rows.map(x=>{const params=new URLSearchParams({section,title:x.title||'',description:x.description||'',icon:x.icon||'•'});if(x.id)params.set('id',x.id);if(x.url)params.set('url',x.url);return `<a class="module-card module-card-link" href="modul.html?${params.toString()}"><span class="module-icon">${esc(x.icon||'•')}</span><h3>${esc(x.title)}</h3><p>${esc(x.description||'')}</p><strong>Lihat kandungan →</strong></a>`}).join('');}
